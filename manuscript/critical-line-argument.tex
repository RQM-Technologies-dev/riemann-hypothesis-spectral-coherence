...
The unresolved burden is to prove, under explicit assumptions, the bridges that feed the reduction:
\begin{enumerate}
  \item \textbf{Target A burden:} establish a rigorous correspondence map with precise domain/codomain control, and exclude spurious balanced states that do not encode zeros.
  \item \textbf{Target B burden:} prove a theorem-level localization statement forcing every admissible balanced point onto $\RePart(s)=\tfrac12$.
  \item \textbf{Global exclusion burden:} upgrade local or regime-restricted arguments to a full-strip exclusion theorem compatible with the assumptions used in Targets A and B.
\end{enumerate}
...
\openbridge{Current manuscript status: the conditional deduction is structurally clear, but Targets A and B and the global off-line exclusion mechanism remain open proof obligations requiring full theorem-level proofs under synchronized assumptions.}
