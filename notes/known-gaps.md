...
- The first named proof pattern is now explicit and consistently labeled across manuscript files: initialization target (`lem:b4-delta-initialization-seed`) + structural non-increase target (`lem:b4-delta-structural-nonincrease`) + synchronized seed-extension coverage \(\Rightarrow\) first control target (`lem:b4-first-delta-control`), before downstream transport/no-drift and neutralizing theorem targets.
